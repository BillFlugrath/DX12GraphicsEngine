...
#include "stdafx.h"
#include "DXPointCloudComputeShader_1.h"
...
DXPointCloudComputeShader_1::DXPointCloudComputeShader_1()
{
}
...
bool DXPointCloudComputeShader_1::Initialize(ComPtr<ID3D12Device>& device, std::shared_ptr<DXDescriptorHeap> descriptor_heap_srv, const std::wstring& filename)
{
...
void  DXPointCloudComputeShader_1::BuildRootSignature()
{
...
void  DXPointCloudComputeShader_1::BuildPSOs()
{
	DXComputeShader::BuildPSOs();
}


void DXPointCloudComputeShader_1::DoComputeWork()
{
	...
	//We wrote data into the mOutputBuffer uav in the compute shader.  To read it back on CPU, we need to copy 
	// the buffer into the mReadBackBuffer buffer.  The buffers must have the same dimension!!  Thus, if 
	// mBuffMap0 is 2D, mReadBackBuffer CopySource will fail!!  Readback buffers are only 1d.
...
//	mCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(mBuffMap0.Get(),
	...
	if (bDebugWriteDataToDisk)
	{
		...
		D3D12_RANGE readbackBufferRange{ 0, NumDataElements * sizeof(TestData) };
...
void DXPointCloudComputeShader_1::BuildResources()
{
...
void DXPointCloudComputeShader_1::BuildDescriptors()
{
